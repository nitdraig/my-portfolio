...
- **Framework:** se mantiene Astro 7. Se descarta migrar a Next.js. El sitio actual ya usa Astro, i18n, prerender y sitemap; migrar implicaría reescribir todo sin beneficio concreto. Three.js se implementará como isla cliente que hidrata solo al interactuar, respetando el principio "HTML first, JS second, WebGL third".
- **Proveedor de IA:** DeepSeek, llamado a través de un proxy server-side. Se usa el SDK compatible con OpenAI para que el proveedor sea intercambiable. La clave nunca viaja al cliente.
- **Flujo opcional:** la experiencia `/idea` nunca bloquea al visitante. Siempre debe existir acceso directo a Proyectos, Proceso, Sobre mí, Recursos y Contacto.
- **URLs:** se mantienen las actuales (`/es/projects/[slug]`, `/en/projects/[slug]`). La experiencia interactiva vive en `/es/idea` y `/en/idea`. No se migra a `/work` para evitar pérdida de indexación actual.
...
- La navegación principal (Work/Process/About/Resources/Contact) debe permanecer visible; el flujo de IA es opcional, no una puerta de entrada obligatoria.
...
- Se mantiene la estructura de URLs existente (`/es/...`, `/en/...`) para no perder indexación actual.
- La experiencia interactiva vive en `/[locale]/idea`.
- Las secciones actuales de la home (About, Services, Process, Testimonials, Contact) tendrán su propia página (`/about`, `/process`, `/contact`) y seguirán siendo accesibles directamente desde la navegación.
...
Implementación: archivos TypeScript en `src/data/` siguiendo el patrón actual de `projects.ts`, `resources.ts` y `projectTaxonomy.ts`.

- `person.ts`: datos de Agustín (nombre, rol, ubicación, idiomas, links).
- `services.ts`: lista de servicios con descripción.
- `process.ts`: etapas del proceso.
- `faq.ts`: preguntas frecuentes verificables.
- `projects.ts`: ampliar el tipo para soportar campos adicionales (`role`, `challenges`, `lessons`, `outcome`, `context`).
- `testimonials.ts`, `skills.ts`, `experience.ts`: según sean necesarios.

La IA debe consultar esa información. Nunca inventar datos fuera de esta base.
...
- Antes del primer envío mostrar un aviso visible: "Tu idea se enviará a DeepSeek, un proveedor de IA externo, para generar una primera estructuración. No la almacenamos en nuestros servidores ni la usamos para analytics."
- No almacenar el texto de la idea en base de datos ni logs.
- No enviar la idea a analytics ni a terceros ajenos al proveedor de IA.
- Permitir al usuario cerrar o abandonar el flujo en cualquier momento.
- La clave del proveedor solo existe en variables de entorno del servidor.
...
- rate limiting (Upstash Redis, reutilizando el patrón del endpoint de contacto);
- protección contra abuso del endpoint de IA (Turnstile o validación equivalente);
- validación de inputs con Zod;
- sanitización del prompt y del texto enviado;
- límite de tamaño del input de idea (por ejemplo, 2000 caracteres);
- timeout del endpoint de IA;
- protección de API keys;
- secretos exclusivamente en servidor;
- CSP cuando sea viable;
- headers de seguridad.

Nunca exponer claves de proveedores de IA en el cliente. El endpoint `/api/idea` debe devolver estructura tipada y rechazar silenciosamente abuso, igual que `/api/contact`.
...
No intentar construir todo simultáneamente. El objetivo es entregar valor de conversión lo antes posible; Motion y Three.js son mejoras progresivas, no bloqueantes.

## Fase 1 — Foundation + IA básica (MVP funcional)

- arquitectura (`src/data/` como fuente de verdad, i18n, layouts);
- contenido estático (About, Services, Process, Resources, FAQs);
- SEO y estructura (`/sitemap.xml`, `/robots.txt`, JSON-LD);
- responsive y accesibilidad base;
- navegación persistente con acceso directo a Proyectos, Proceso, Sobre mí, Recursos y Contacto;
- portfolio: ampliar `Project` para soportar campos del caso de estudio;
- contacto directo funcional;
- **experiencia `/idea` mínima viable**: input del usuario → endpoint `/api/idea` con DeepSeek → análisis estructurado (problema/usuarios/MVP) → CTA a contacto con contexto precargado.

## Fase 2 — Mejoras de conversión

- resumen del proyecto antes del contacto;
- formulario contextual que reutilice la idea;
- portfolio contextual (relacionar la idea con proyectos existentes por tags/industria);
- analytics mínimos (sin registrar el contenido de la idea).

## Fase 3 — Motion y refinamiento visual
...
## Fase 4 — Three.js (lazy, opcional)

- escena abstracta liviana en `/idea`;
- sistema de nodos para la descomposición;
- interacción hover/click/teclado;
- responsive;
- fallback 2D;
- `prefers-reduced-motion` y detección de WebGL/GPU.
...
- [ ] Se puede explorar el sitio y contactar sin usar el flujo de IA.
- [ ] El aviso de privacidad de IA es visible antes del primer envío.
- [ ] La interacción 3D tiene propósito (cuando exista).
- [ ] La web funciona sin WebGL.
- [ ] La web funciona sin JavaScript para contenido estático.
...
- [ ] Endpoint `/api/idea` con rate limit, timeout y validación Zod.
- [ ] Clave de DeepSeek solo en servidor.
- [ ] Aviso de privacidad visible antes del envío a IA.
- [ ] No se registra ni almacena el contenido de la idea.
- [ ] CSP y headers de seguridad configurados.
